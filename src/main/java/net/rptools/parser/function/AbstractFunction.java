...
public abstract class AbstractFunction implements Function {
    ...
    private String formatExpectedParameterString() {
        if (minParameters == maxParameters) return String.format("exactly %d parameter(s)", maxParameters);

        if (maxParameters == UNLIMITED_PARAMETERS) return String.format("at least %d parameters", minParameters);
